...
object FiveNumberSummary {
    ...
    //def AtoBYearData(): Unit = {
        ...
        val stats = delaysPerDate.map { case(date, delays) => 
                ...
                (date, (min, max, median, Q1, Q3, avg, count, outlierThreshold, outliers ) ) }
        ...
        val nonZeroDelaysPerDate: RDD[(String, Array[Double])] = delaysPerDate.map { case(k,v) => (k, v.filter(x => x > 0).toArray ) }
